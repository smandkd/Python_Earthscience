...
def sw_dens(S, T):
    ...
    # smow_T = sw.smow(T)
    smow_T = (999.842594 + (6.793952e-2 + (-9.095290e-3 + (1.001685e-4 + (-1.120083e-6 + 6.536332e-9*T68)*T68)*T68)*T68)*T68)
...
def mixing_depth(D, T, S, Vmax, TS, R):
    ...
    SN = 0.4 # S number - nondimensional storm speed
    ...
    for d in range(1, len(depth_range)):
        d_idx = d - 1
        mean_density = np.nanmean(dens[:d])
        richardson_num = (((9.8 * d_idx) * (dens[d_idx] - mean_density)) / (p0 * (SN * (Tx / (p0 * d_idx)) * FT) ** 2))
        if richardson_num >= 0.6:
            Dmix = depth_range[d_idx]
            # 결과를 시각화
            break    
