...
!
  ...
  SUBROUTINE znot_m_v7(uref,znotm)                ! lwj 2018.9.4.
  IMPLICIT NONE
...
!
    ...
    REAL             :: bs0, bs1, bs2, bs3, bs4, bs5, bs6
    REAL             :: cf0, cf1, cf2, cf3, cf4, cf5, cf6
    REAL             :: aa0, aa1, aa2, aa3, aa4,aa5, aa6
    ...
    aa0 =  6.55768835798245e-12
    aa1 = -2.58858365218131e-09
    aa2 =  4.21453985140560e-07
    aa3 = -3.62617643138979e-05
    aa4 =  0.00174340507684587
    aa5 = -0.0446490827573998
    aa6 =  0.480617832752136

    IF ( uref .LE. 5.0 ) THEN
      znotm = (0.0185 / 9.8*(7.59e-4*uref**2+2.46e-2*uref)**2)
    ELSEIF (uref .GT. 5.0 .AND. uref .LT. 10.0) THEN
      znotm =.00000235*(uref**2 - 25 ) + 3.805129199617346e-05
    ELSEIF ( uref .GE. 10.0  .AND. uref .LT. 40.0) THEN
      znotm = bs6 + bs5*uref + bs4*uref**2 + bs3*uref**3 + bs2*uref**4 +  &
              bs1*uref**5 + bs0*uref**6
    ELSEIF ( uref .GE. 40.0  .AND. uref .LE. 80.0) THEN
      znotm = aa6 + aa5*uref + aa4*uref**2 + aa3*uref**3 + aa2*uref**4 +  &
              aa1*uref**5 + aa0*uref**6
    ELSEIF ( uref .GT. 80.0 ) THEN
      znotm = 3.352062565298297e-06
    END IF

    END SUBROUTINE znot_m_v7
  SUBROUTINE znot_m_v8(uref,znotm)
  IMPLICIT NONE
! modified by SUNGHUN Kim , decreasing Cd 
! Calculate areodynamical roughness over water with input 10-m wind
! For low-to-moderate winds, try to match the Cd-U10 relationship from COARE
! V3.5 (Edson et al. 2013)
! For high winds, try to fit available observational data
!
! Bin Liu, NOAA/NCEP/EMC 2017
! 
...
!
    ...
    elseif (uref > 6.5 .and. uref <= 15.7) then
      znotm = p25*uref**5 + p24*uref**4 + p23*uref**3 + p22*uref**2 + p21*uref + p20
    elseif (uref > 15.7 .and. uref <= 46.0) then
      znotm = exp( p35*uref**5 + p34*uref**4 + p33*uref**3 + p32*uref**2 + p31*uref + p30 )
    elseif ( uref > 46.0) then
      znotm = 10 * (1/exp(sqrt(0.16 /( p45*uref**5 + p44*uref**4 + p43*uref**3 + p42*uref**2 + p41*uref + p40 ))))
    else
      print*, 'Wrong input uref value:',uref
    endif
...
!ch
          tmp=0.4*0.4/(alog(zlevt/zm)*alog(zlevt/zt))  ! ch at zlev using old formula
            zt1=z10/exp( 0.4*0.4/( 1.0*tmp*alog(z10/zm1) )  )
             ...
             write(0,*)'stop, icoef_sf must be one of 0,1,2,3,4,5,6'
             stop
          endif
          znott=zt1
          znotm=zm1
